  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5' >
        ...
        <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showfliter ? 'bg-primary text-white':''}`} 
        onClick={() => setshowfilter(prev => !prev)} >Filters</button>


        <div className={`  flex-col gap-4 text-sm text-gray-600 ${ showfliter ?'flex': ' hidden sm:flex' } `} >
          <p onClick={() => { speciality === 'General physician' ? navigate('/doctors') : navigate('/doctors/General physician'); console.log(speciality) }}
